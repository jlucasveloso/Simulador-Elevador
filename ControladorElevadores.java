public class ControladorElevadores {
    ...
    public ControladorElevadores(Configuracao config) {
        ...
        this.filas = new FilaPrioridadeDupla[config.numeroAndares];
        for (int i = 0; i < config.numeroAndares; i++) {
    ...
    public void gerarPessoas() {
        ...
        for (int i = 0; i < quantidade; i++) {
            int andarOrigem = (int)(Math.random() * config.numeroAndares);
            int andarDestino = (int)(Math.random() * config.numeroAndares);
            while (andarDestino == andarOrigem) {
                andarDestino = (int)(Math.random() * config.numeroAndares);
            }
            ...
            filas[andarOrigem].inserir(nova);
    ...
    private void atualizarPainelControle() {
        for (int andar = 0; andar < config.numeroAndares; andar++) {
